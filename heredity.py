    ...
    for person in people:
        ...
        if people[person]["father"] in two_genes and people[person]["mother"] in two_genes:
            ...
            if person in one_gene:
                ...
                #inherit no genes
                probabilities[person] = PROBS["mutation"] * PROBS["mutation"] # .0001 # double checked
        ...
        # one parent has two genes, the other has one gene
            ...
            if person in two_genes:
                # probabilities[person] = (1 - PROBS["mutation"]) * (.5 * (1 - PROBS["mutation"]) + (.5 * PROBS["mutation"])) # (.99 * (.5 * .99) + (.5 * .01)) = 0.49505
                # (.99 * .5 *.99) + (.99 * .5 * .01) = .495
                probabilities[person] = .99 * .5 * .99 # = .49005, double checked
            if person in one_gene:
                ...
                probabilities[person] = (.99 * .5 * .49) + (.01 * .5 * .49)#  = .245 # not really sure about this one
            else:
                #inherit no genes
                ...
                probabilities[person] = (.01 * .5 * .49) + (.01 * .5 * .49) # = .0049 **duck says .01 * .5 * .99 = .00495
        ...
        # one parent has two genes, the other has none
            ...
            or (people[person]["mother"] in two_genes and (people[person]["father"]  not in two_genes and people[person]["father"] not in one_gene)):
                ...
                if person in one_gene:
                    #one gene
                    probabilities[person] =  (.99 * .99) + (.01 * .01) # = 0.9802 <-- use this as a guide (comes from heredity spec), duck agreed. double checked
                    # (.99 * .5 *.99) + (.01 * .5 * .01) # = 0.4901
                else:
                    #person has no genes
                    probabilities[person] = .01 * .99 # = .0099
        ...
        # both parents have one_gene
            ...
            if person in one_gene:
                probabilities[person] = (.5 * .99) * (.5 * .99) # =.245025
                # inherit one gene
            else:
        ...
        # one parent has one gene, the other parent has none
            ...
            if person in two_genes:
                probabilities[person] = .5 * .99 * .01
            if person in one_gene:
                probabilities[person] = (.99 * .5 * .99) + (.01 * .5 * .01) # = .4901
                # inherit one gene
            else:
                #inherit no genes
                probabilities[person] =(.5 *.01 * .99) + (.5 * .99 * .99) # = .495
                #  .5 + .01 * .5 =
        ...
        # neither parent has gene (will this run for people without parents? if it does it will )
            ...
            and people[person]["mother"] not in two_genes and people[person]["father"] != None):
            print("this person,", person, "should have parents")
            if person in two_genes:
                probabilities[person] = (.01 * .01)
            if person in one_gene:
                probabilities[person] = (.01 * .99) + (.01 * .99)
                # inherit one gene
