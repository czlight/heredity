    ...
    for person in people:
        ...
        # one parent has two genes, the other has one gene
            ...
            if person in one_gene:
                ...
                probabilities[person] = (.5 * (1 - PROBS["mutation"]) * (.5 * PROBS["mutation"])) + (.5 * PROBS["mutation"]) * (.5 * (1 - PROBS["mutation"])) # (.5 * .99 * (.5 * .01) + ((.5 * .01) * (.5 * .99) ) = (.002475) + ()
                # (.99 * .5 * .49) + (.01 * .5 * .49) = (.24255) + (.00245) = 0.245
            else:
                #inherit no genes
        ...
        # one parent has one gene, the other parent has none
            ...
            if person in one_gene:
                probabilities[person] = (.99 * .5 * .99) + (.01 * .5 * .01)
                # inherit one gene
            else:
                #inherit no genes
                probabilities[person] =(.5 * .01 * .99) + (.5 * .99 * .01)
